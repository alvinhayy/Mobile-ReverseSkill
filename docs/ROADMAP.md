...
| 5 | **Advanced Flutter TLS (when standard scripts fail)** | `runtime/flutter-tls-connect-redirect.js` + Ghidra note in `docs/` | Dart ignores the system proxy → hook `getaddrinfo`/`connect` + inject a `CONNECT` verb; Ghidra-patch `ssl_..._session_verify_cert_chain`; **gotcha: needs a physical ARM64 device** (x86_64 emulator hides `libflutter.so` behind `libndk_translation`). Directly serves the repo's Flutter targets. | ☐ |
...
| 10 | **Flutter root/JB-detection bypass** | `runtime/flutter-jb-root-bypass.js` | `flutter_jailbreak_detection` delegates to RootBeer / IOSSecuritySuite over a **MethodChannel** — bypass at the channel gap or a ~12-line smali patch. | ☐ |
...
| Flutter **manual-offset unpin fallback** (Ghidra locate `ssl_..._verify_cert_chain`) | note in `docs/bypass-reference.md` | ☐ |
