...
5. **Spawn in the background**:
   `nohup "<emulator>" -avd "<name>" -no-snapshot-load >/tmp/emu-<name>.log 2>&1 &`
6. **Wait for boot**:
   `adb wait-for-device` then poll `adb shell getprop sys.boot_completed` until it returns `1`.
